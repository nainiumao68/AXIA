"""Image transforms used by the data pipeline (crop / pad / normalize)."""
import random
...
def normalize(img, mean, std):
    # Pretrained backbones expect inputs in the 0-1 range.
    img = img.astype(np.float64) / 255.0

    mean = np.array(mean)
    std = np.array(std)

    # For 3D images (H, W, C), broadcast mean/std over the channel axis.
    if len(img.shape) == 3:
